<x-app-layout>
    <div class="min-h-[calc(100vh-65px)] flex flex-col sm:justify-center items-center bg-gray-100">
        ...
        <div class="sm:max-w-3xl w-full px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-l-sm my-12">
            ...
            <hr class="mb-4">
            <form method="post" enctype="multipart/form-data" action="{{ route('admin.rooms.store', ['hotel' => $hotel]) }}">
                @csrf
                ...
                <div>
                    <label class="placeholder-box">
                        <div class="flex justify-center mb-4">
                            <img class="thumbnail-sm rounded-l-sm" id="preview" src="{{ asset('storage/default.jpg') }}" alt="Изображение номера">
                        </div>
                        <div>
                ...
                <div>
                    <label class="placeholder-box">
                        <input type="text" class="shadow appearance-none border rounded-l-sm w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" maxlength="100" name="name" value="{{ old('name') }}" required>
                ...
                <div>
                    <label class="placeholder-box">
                        <textarea class="shadow appearance-none border rounded-l-sm w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="10" name="description" required>{{ old('description') }}</textarea>
                ...
                <div>
                    <label class="placeholder-box">
                        <input type="text" class="shadow appearance-none border rounded-l-sm w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" maxlength="100" name="floorArea" value="{{ old('floorArea') }}" required>
                ...
                <div>
                    <label class="placeholder-box">
                        <input type="text" class="shadow appearance-none border rounded-l-sm w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" maxlength="100" name="type" value="{{ old('type') }}" required>
                ...
                <div>
                    <label class="placeholder-box">
                        <input type="text" class="shadow appearance-none border rounded-l-sm w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" maxlength="100" name="price" value="{{ old('price') }}" required>
                    ...
                    @foreach($facilities as $facility)
                        <label for="{{ 'facility' . $facility->id }}" class="inline-flex items-center">
                            <input
                                ...
                                value="{{ $facility->id }}"
                                @if(isset(old('checkedFacilities')[$facility->id])) checked @endif
